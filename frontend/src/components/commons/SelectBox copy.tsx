...
export default function SelectBox({
    ...
    return (
        ...
        <select
            ...
            name={name}
            className={`
                flex-none border appearance-auto border-gray-300 dark:border-gray-900 text-[#808EA1] dark:bg-[#444444] dark:text-white 
                hover:border-[#434343] transition duration-300 text-sm rounded-lg block p-2.5 px-4 shadow-md 
                ${centerText ? 'text-center' : ''} 
                ${disable ? 'bg-[#e7e9eb] dark:bg-[#333333] cursor-not-allowed' : ''} 
                focus:outline-none focus:ring-0 ${customClassName} 
                ${label ? 'pt-5' : ''} 
            `}
        ...
        >
            {loading ? (
                <option value="-9999">Đang tải...</option>
            ) : (
                <>
